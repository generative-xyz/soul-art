...
const useCreateAuction: ContractOperationHook<
...
> = () => {
  ...
  const estimateGas = useCallback(
    async (params: ICreateAuctionParams): Promise<string> => {
      if (account && provider && contract) {
        const { tokenId } = params;
        const gasLimit = await contract.estimateGas.createAuction(tokenId);
  ...
  const call = useCallback(
    async (params: ICreateAuctionParams): Promise<Transaction | null> => {
      if (account && provider && contract) {
        logger.debug('useCreateAuction', params);
  ...
  return {
    ...
    operationName: 'Start Adopt Proposal',
  };
};
